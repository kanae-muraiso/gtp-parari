...
function removeEmptyTextPanelsBeforeSave(input: string): string {
  const blocks = parseBlocks(String(input ?? ""));
  ...
  return serializeBlocks(keptBlocks);
}
...
export default function BookPanelSequenceEditorPage() {
  ...
  const handleSave = async () => {
    ...
    if (!row) {
      ...
      const rawSaveSsot = latestSsotRef.current;

      const saveSsot = isWebLikeSsot(rawSaveSsot)
        ? rawSaveSsot
        : removeEmptyTextPanelsBeforeSave(rawSaveSsot);

      if (saveSsot !== rawSaveSsot) {
        latestSsotRef.current = saveSsot;
        setSsot(saveSsot);
      }
 ...
 const webWarnings = getWebStructureWarnings(ssot);
  ...
  title="STUDIO · 編集中"
    ...
    <main className="min-h-screen bg-emerald-50 pt-11">
      {status.type === "error" ? (
