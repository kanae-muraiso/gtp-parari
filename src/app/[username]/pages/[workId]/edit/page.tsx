...
export default function UserPageEditPage() {
  ...
  const loadPageWork = useCallback(async () => {
    ...
    if (profile.user_id !== user.id) {
      setStatus({
        type: "error",
        message:
          "このPAGEは編集できません。ログイン中ユーザーとURLのユーザーが一致しません。",
      });
      return;
    }
        ...
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-3">
          <div>
            <div className="text-xs font-semibold text-neutral-400">
              /{routeUsername}/pages/{workId}/edit
            </div>
        ...
        <>
          <div className="mx-auto max-w-5xl px-4 pt-4">
            <div className="rounded-3xl border border-sky-200 bg-sky-50 p-4 text-sm leading-6 text-sky-900">
              <div className="font-semibold">PAGE編集</div>
              <p>
                この画面は <code>[PAGE]</code>{" "}
                で始まる新PAGE作品だけを編集します。旧BOOK作品は旧エディタ側で扱います。
              </p>
           ...
           <PageEditor
             ...
             viewHref={
               row?.stable_slug ? `/${routeUsername}/${row.stable_slug}` : undefined
             }
