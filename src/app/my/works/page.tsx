...
export default function MyWorksPage() {
  ...
  const [deletingWorkIds, setDeletingWorkIds] = React.useState<string[]>([]);
    ...
    const [categories, setCategories] =
      React.useState<WorkCategoryRow[]>([]);
    const [categoryLinks, setCategoryLinks] =
      React.useState<WorkCategoryLinkRow[]>([]);
    ...
    const loadWorks = async () => {
      ...
      setCurrentUserId(user.id);
        ...
        if (categoryLinkError) {
          console.warn(
            "[my/works] category link load failed:",
            categoryLinkError,
          );
        }
  ...
  const onDelete = React.useCallback(
    ...
    const categoryIdsByWork = React.useMemo(() => {
      const map = new Map<string, string[]>();

      for (const link of categoryLinks) {
        const ids = map.get(link.work_id) ?? [];
    ...
    const uncategorizedCount = React.useMemo(
      () =>
        works.filter(
          (work) =>
            (categoryIdsByWork.get(work.id) ?? []).length === 0,
        ).length,
      [works, categoryIdsByWork],
    );
    ...
    const filteredWorks = React.useMemo(() => {
      ...
      return works.filter((work) =>
        (categoryIdsByWork.get(work.id) ?? []).includes(
          activeCategoryId,
        ),
      );
    }, [
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-wrap sm:justify-end">
                        ...
                        <Link
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex w-full justify-center rounded-full bg-blue-50 px-2.5 py-1.5 text-[11px] font-bold text-blue-700 ring-1 ring-blue-200 transition hover:bg-blue-100 sm:w-auto"
                        >
                          表示
                        </Link>
